...
def process_images(input_folder, output_folder, metadata_file, target_size=512, force_reprocess=False):
    ...
    for img_file in tqdm(image_files, desc="处理图片"):
        ...
        try:
            ...
            # 自动判断术前/术后（根据文件名）
            is_preop = "术前"
            if any(keyword in img_file.lower() for keyword in ["post", "after", "术后", "postop"]):
                is_preop = "术后"
            
            # 保存处理后的图片
            output_filename = f"{image_id}_{is_preop}_{img_file}"
...
if __name__ == "__main__":
    ...
    # 直接使用绝对路径
    input_folder = r"D:\upm_model train\唇裂项目\原始图片"
    output_folder = r"D:\upm_model train\唇裂项目\处理后图片"
    metadata_file = r"D:\upm_model train\唇裂项目\图像元数据.csv"
    ...
    try:
        if os.path.exists(metadata_file):
            ...
            preop_count = len(metadata[metadata['术前/术后'] == '术前'])
            postop_count = len(metadata[metadata['术前/术后'] == '术后'])
            print(f"处理结果统计: 术前图片 {preop_count} 张，术后图片 {postop_count} 张，共 {len(metadata)} 张")
    except Exception as e:
